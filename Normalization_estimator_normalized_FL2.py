"""
Estimate AUCSTD using max-normalized FL2 (R18) loading traces.

This is an optional alternative to `Normalization_estimator.py`. The input
files and filtering logic are the same, but the FL2 loading trace for each
sample is first divided by its own loading-window maximum before AUC is
computed. This reduces dependence on absolute FL2 intensity while preserving
the shape-based loading correction.
...
def normalized_fl2_auc(series: pd.Series, window: int = AUC_WINDOW) -> float:
    ...
    return float(np.trapz(loading_window / max_value))
...
def auc_gold(loading_fl2: pd.DataFrame, good_mask: pd.Series, window: int = AUC_WINDOW) -> float:
    """Estimate gold-standard normalized FL2 AUC among baseline-qualified traces."""
    auc_golds = []
    for col in loading_fl2.columns:
        if bool(good_mask.get(col, False)):
            auc_tmp = normalized_fl2_auc(loading_fl2[col], window=window)
            if not pd.isna(auc_tmp):
                auc_golds.append(auc_tmp)

    if not auc_golds:
        raise ValueError("No traces passed baseline filter; cannot estimate AUC_std.")

    auc_std = float(np.median(auc_golds))
    print(f"Normalized FL2 AUC_std (median of good traces): {auc_std:.4f}")
    return auc_std


def normalize_fl1(fl1_original: pd.DataFrame, fl2_original: pd.DataFrame,
                  auc_std: float, window: int = AUC_WINDOW) -> pd.DataFrame:
    """Apply normalized-FL2-AUC correction factor to each FL1 trace."""
    ...
    for col in common_cols:
        auc_tmp = normalized_fl2_auc(fl2_clean[col], window=window)
        if pd.isna(auc_tmp):
            fl1_norm[col] = np.nan
            continue
...
def plot_all(fl1_to_plot: pd.DataFrame, fl2_to_plot: pd.DataFrame,
    ...
    for col in fl2_clean.columns:
        auc_by_trace[col] = normalized_fl2_auc(fl2_clean[col], window=auc_window)
        ...
        max_value = fl2_clean[col].dropna().iloc[:auc_window].max()
        fl2_plot = fl2_clean[col].iloc[:plot_window]
        if not pd.isna(max_value) and max_value != 0:
            fl2_plot = fl2_plot / max_value

        color = "black"
        alpha = 0.5
        axes[0, 0].plot(fl2_plot.values, color=color, alpha=alpha)
        axes[0, 0].set_title("R18 FL2 (max-normalized)")
        ...
        axes[1, 1].set_title("HeadP_Ab Fluor, FL1 (post-normalized-FL2-AUC)")

    axes[1, 0].axis("off")

    auc_summary = pd.concat(
        [
            pd.Series(auc_by_trace, name="Normalized FL2 AUC"),
    ...
    auc_summary.to_excel(export_xlsx.replace(".xlsx", "_normalized_FL2_AUCplot.xlsx"))

    baseline_export_df = pd.DataFrame(baseline_export).mean(axis=0)
    baseline_export_df.to_excel(export_xlsx)

    fig.tight_layout()
    fig.savefig(export_xlsx.replace(".xlsx", "_normalized_FL2_traces.png"), dpi=300)
    if show_plot:
        plt.show()
    else:
        plt.close(fig)
...
def _build_parser():
    """Create CLI parser for normalized-FL2-AUC workflow."""
    parser = argparse.ArgumentParser(
        description="Normalize HeadP_Ab FL1 traces using max-normalized FL2 loading AUC."
    )
    parser.add_argument(
        "--folder",
        required=True,
...
def main():
    ...
    trace_fl1, loading_fl2, base_fl1, _loading_fl1 = acquire_data(
        sub_folders,
        args.phase,
